...
# Window number for TCL commands
WINDOW_NUMBER = 6
...
def generate_vias_for_row(x_min, x_max, y_pos):
    ...
    while current_x <= x_end:
        ...
        via_commands.append("gi::setField {{viaAuto}} -value {{false}} -in [gi::getToolbars {{deCommandOptions}} -from [gi::getWindows {}]]".format(WINDOW_NUMBER))
        via_commands.append("gi::setField {{viaDefName}} -value {{{}}} -in [gi::getToolbars {{deCommandOptions}} -from [gi::getWindows {}]]".format(VIA_TYPE, WINDOW_NUMBER))
        via_commands.append("de::addPoint {{{:.3f} {:.3f}}} -context [db::getNext [de::getContexts -window {}]]".format(current_x, y_via, WINDOW_NUMBER))
        via_commands.append("")
...
def generate_tcl_script(components, nets, output_path):
    ...
    tcl_commands.append("# Window number: {}".format(WINDOW_NUMBER))
    tcl_commands.append("#")
    ...
    # Process Ntap rows (connected to power)
...
if __name__ == "__main__":
    input_path = r"C:\Users\TEBA\Desktop\Neat Routing Automation\DEF_file_Input.txt"
    output_path = r"C:\Users\TEBA\Desktop\Neat Routing Automation\taps routing.tcl"
